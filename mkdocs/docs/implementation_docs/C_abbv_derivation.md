...
An initial guess for the cut-off time $T$ is not necessary: the first iteration uses $Q_o = 1$.
...
All the components of $\vec a_T(t)$ have been found. (C.6.1) can now be used in a numerical integrator to solve for $v_{\theta}(T)$.
